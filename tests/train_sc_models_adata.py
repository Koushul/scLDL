...
def main():
    ...
    # Preprocessing
    print("Preprocessing...")
    # sc.pp.filter_cells(adata, min_genes=200) # Optional for this cleaner dataset? Let's verify size.
    # sc.pp.filter_genes(adata, min_cells=3)
    
    # Check if already normalized? "normalized_count" layer exists.
    # But usually adata.X is raw counts or processed.
    # Let's assume raw or just re-normalize to be safe/consistent.
    if np.max(adata.X) > 100: # Heuristic for counts
    ...
    # Save Metadata
    os.makedirs('models/sc_adata', exist_ok=True)
    with open('models/sc_adata/preprocessing_metadata.pkl', 'wb') as f:
        pickle.dump({
            'le': le,
    ...
    torch.save(mlp.state_dict(), 'models/sc_adata/MLPBaseline.pth')
    ...
    torch.save(hybrid.state_dict(), 'models/sc_adata/HybridLEVI.pth')
    # Eval
    probs = hybrid.predict(X_test, np.ones((len(X_test), n_classes))/n_classes) 
    acc = np.mean(np.argmax(probs, axis=1) == y_test)
    print(f"HybridLEVI Acc: {acc*100:.2f}%")
    
    # 3. ConcentrationsLE (Re-init to be sure)
    ...
    torch.save(conc.state_dict(), 'models/sc_adata/ConcentrationLE.pth')
    # Eval
    ...
    torch.save(impr.state_dict(), 'models/sc_adata/ImprovedLEVI.pth')
    # Eval
