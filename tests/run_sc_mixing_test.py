...
def load_data_and_models():
    # Load Metadata
    with open('models/sc_tonsil/preprocessing_metadata.pkl', 'rb') as f:
        meta = pickle.load(f)
    ...
    # Load Indices
    with open('models/sc_tonsil/split_indices.pkl', 'rb') as f:
        splits = pickle.load(f)
    test_indices = splits['test']
    
    # Slice to Test Set ONLY
    X = X[test_indices]
    y = y[test_indices]
    ...
    m.load_state_dict(torch.load('models/sc_tonsil/MLPBaseline.pth', map_location=DEVICE))
    m.eval()
    ...
    m.load_state_dict(torch.load('models/sc_tonsil/HybridLEVI.pth', map_location=DEVICE))
    m.eval()
    models['HybridLEVI'] = m
    
    print("Loading ConcentrationLE...")
    m = ConcentrationLE(n_features, n_classes, encoder_type='mlp', n_hidden=256, device=DEVICE)
    m.load_state_dict(torch.load('models/sc_tonsil/ConcentrationLE.pth', map_location=DEVICE))
    m.eval()
    ...
    m.load_state_dict(torch.load('models/sc_tonsil/ImprovedLEVI.pth', map_location=DEVICE))
    m.eval()
...
def main():
    ...
    print(f"\nStarting Evaluation ({args.mix}-Way Mixing scRNA-seq - Tonsil Strict Test Split)...")
    ...
    for batch_idx, (inputs, true_labels) in enumerate(tqdm(dataloader, desc="Evaluating")):
        ...
        if batch_idx == 0:
            ...
            plt.tight_layout()
            plt.savefig(f'reports/plots/sc_tonsil_mixing_{args.mix}way.png')
            plt.close()
            
    print(f"\n=== scRNA-seq MIXING TEST RESULTS (Tonsil Strict Split, 50k Samples, {args.mix}-Way) ===")
